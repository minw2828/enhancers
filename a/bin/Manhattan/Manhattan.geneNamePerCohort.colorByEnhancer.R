...
infile1  <- args[1]  # GWAS result within gene region 
...
getLoopOverList <- function(data1, data2) {
    ...
    dt <- setDT(expand.grid(genders, phenos, chrNs, dbs))
    setnames(dt, c('gender', 'pheno', 'chrN', 'db'))
    return(dt[])
}
...
plot_Manhattan_geneName_db <- function(data, Gender, Pheno, chrN, Db, base, verysignifline, genomewideline, suggestiveline) {
    Title <- paste(Gender, Pheno, Db, sep =', ')
    Xlab  <- paste('chromosome', chrN, sep = ': ')
    Ylab  <- '-log10(p-value)'
    legend_name_colour <- "p-value thresholds & boolean enhancer: "
    g <- ggplot(data = data) +
         geom_point(mapping = aes(x = manhattanPos, y = log_transform(pvalue, base), colour = boolEnhancer), size = 0.5) +
...
get1plot <- function(data1, data2, loopList, index, logBase, verysignifline, genomewideline, suggestiveline) {
    ...
    g <- plot_Manhattan_geneName_db(data, oneRow$gender, oneRow$pheno, oneRow$chrN, oneRow$db, logBase, verysignifline, genomewideline, suggestiveline)
    return(g)
}
...
draw_plots <- function(data1, data2, loopList, geneName, subPlotIndex, nc, logBase, verysignifline, genomewideline, suggestiveline, outfile) {
    Title    <- paste('Manhattan plot zoom in to', geneName, 'gene region')
