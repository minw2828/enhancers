...
getTitle <- function(pheno, db, tpe) {
    if (tpe == 'All') {
        Title <- paste(pheno, paste(tpe, db, sep = " "), sep = ": ")
    } else if ( tpe == 'specific') {
        Title <- paste(pheno, paste(db, tpe, sep = "-"), sep = ": ")
    } else {
        Title <- gsub('_', ' & ', paste(pheno, db, sep = ": "))
    }
    return(Title)
}

plot_histogram <- function(pheno, db, tpe, NbSigSnpsPermutaion, NbSigSnpOriginal, sigThreshold, lowFill, highFill, lineColor) {
    Title <- getTitle(pheno, db, tpe)
    bin_size <- 1
...
get1plot <- function(data, loopList, index, pSigLevel, ntimes) {
    ...
    dat <- data[pheno == oneRow$pheno & db == oneRow$db & laps == oneRow$laps]
    NbSigSnpOriginal    <- dat[tpe == 'ori']$NbSigSnp
    NbSigSnpsPermutaion <- dat[tpe != 'ori']
    g <- plot_histogram(oneRow$pheno, oneRow$db, oneRow$laps, NbSigSnpsPermutaion, NbSigSnpOriginal, pSigLevel, oneRow$lowFill, oneRow$highFill, oneRow$lineColor)
    return(g)
}
