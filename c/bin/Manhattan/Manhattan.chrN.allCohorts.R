...
# outfile1 <- '/group/dairy/Min/geno2pheno/analyses/2016-01-19/a/out/GWAS/Manhattan.annotate_snp_gene_enhancer.NACohorts/output.05.10.2.a.2016-01-19.a.png'
...
chr          <- as.numeric(f22(outfile1, '[.]', 7)[1])
...
subsetByPheno <- function(data, Pheno) {
    res <- data[pheno == Pheno]
    return(res)
}
...
getMinusLogBasePvalue <- function(data, logBase) {
    # after getMinPmeta
    data[, minusLogBasePvalue := -log(x = pmeta, base = logBase)]
    return(data)
}
...
getReadyForPlot <- function(data1, Pheno, logBase) {
    data <- subsetByPheno(data1, Pheno)
    getMinPmeta(data)
    getMinusLogBasePvalue(data, logBase)
    getManhattanPos(data)
    res <- getColourList(data)
    return(res)
}

plot_Manhattan_allCohorts <- function(data1, Pheno, logBase, verysignifline, genomewideline, suggestiveline) {
    data <- getReadyForPlot(data1, Pheno, logBase)
    Title <- Pheno
    Xlab  <- paste('chromosome', paste(unique(data$chr), collapse = ', '))
    Ylab  <- '-log10(p-value)'
    g <- ggplot(data = data) +
...
getLoopOverList <- function(data1) {
    dt <- unique(data1[, .(pheno)])
    return(dt)
}

draw_plots <- function(data1, loopList, logBase, verysignifline, genomewideline, suggestiveline, nc, subPlotIndex, outfile) {
    Title    <- 'Manhattan plots: Meta-analysis'
    ...
    for (i in seq(3, nr)) {
        index <- i - 2
        g <- plot_Manhattan_allCohorts(data1, loopList[index]$pheno, logBase, verysignifline, genomewideline, suggestiveline)
...
run <- function() {
    ...
    draw_plots(data1, loopList, logBase, verysignifline, genomewideline, suggestiveline, nc, subPlotIndex, outfile1)
}
