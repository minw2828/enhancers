...
check_geneFound <- function(data1, target) {
    geneNames <- data1[chromosome == target$Chromosome]$geneName
    ...
    result <- data.table(geneName = geneNames, 
                         Gender = target$Gender,
                         Phenotype = target$Phenotype,
                         Chromosome = target$Chromosome,
                         Database = target$Database,
                         boolFound = res) # 0: not found; 1: found 
    return(result)
}

result <- function(data1, data2) {
    setkey(data1, geneName)
    setkey(data2, geneName)
...
## run ##
run <- function() {
    data1 <- setnames(fread(infile1), c('geneName', 'chromosome', 'geneStart', 'geneEnd', 'description'))
    data2 <- fread(infile2)
