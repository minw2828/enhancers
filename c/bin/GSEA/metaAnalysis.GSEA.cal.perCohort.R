...
calculate_enrichment_score <- function(dat) {
    nFALSE <- nrow(dat[which(!boolenhancer)])                    # Don't use boolenhancer == TRUE, it will cause errors
    nR     <- sum(dat[which(boolenhancer)]$minusLog10Pmeta)
...
cal_ES_significance <- function(esNulls, lesESnp, ES, round2) {
    ...
    if (nrow(lesESnp) > 0 & ES.rank == nr.mdt.sorted) {
        res <- paste("<", round(1/nr.mdt.sorted, round2), sep = "")
    } else {
        res <- round((nr.mdt.sorted - ES.rank)/nr.mdt.sorted, round2)
    }
    return(res)
}
...
run <- function() {
    ...
    ES.rank <- cal_ES_significance(esNulls, lesESnp, ES, 6)
