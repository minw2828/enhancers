...
outfile1 <- args[4]
outfile2 <- args[5]
# infile1  <- '/group/dairy/Min/geno2pheno/analyses/2016-01-19/c/out/GSEA/metaAnalysis.GSEA/snpName_pheno_chrN_snpeffectmeta_varmeta_zmeta_pmeta_minusLog10Pmeta_source_boolpromoter_pHitMiss_accSum.2016-01-19.c.tab'
...
#    Title <- 'Running Sum: sorted -log10(p-value) (largest -> smallest)'
    ...
    legend_name_fill <- "boolean promoter"
    data[, rIndex := data[,.I]]
    g <- ggplot(data = data) +
         ...
         geom_rect(mapping = aes(xmin = rIndex, xmax = rIndex, ymin = max(accSum)+max(accSum)/10, ymax = Inf, fill = as.factor(boolpromoter)), size = 1) +
...
## generate historgram of variance: (promoter) & (promoter & non-promoter) ##
plot_histogram_variance_boolpromoter <- function(data) {
    ...
    legend_name_fill <- "boolean promoter"
    g <- ggplot(data = data) +
         geom_histogram(mapping = aes(variance, fill = boolpromoter), binwidth = bin_size) +
         facet_wrap(~ boolpromoter, ncol = 1) +
         guides(fill = guide_legend(title = legend_name_fill)) +
         labs(title = Title, x = Xlab, y = Ylab) +
         get_Theme()
    return(g)
...
plot_boxplot_varianceRank_boolpromoter <- function(data) {
    Title <- 'Boxplot: -log10(p-value) Rank'
    Xlab  <- "boolean promoter"
    Ylab  <- "-log10(p-value) Rank"
    legend_name_fill <- "boolean promoter"
    data[, rIndex := data[,.I]]
    g <- ggplot(data = data) +
         geom_boxplot(mapping = aes(boolpromoter, rIndex, fill = boolpromoter)) +
         guides(fill = guide_legend(title = legend_name_fill)) +
         labs(title = Title, x = Xlab, y = Ylab) +
         get_Theme()
    return(g)
...
plot_pHitMiss <- function(data) {
    ...
    data[, rIndex := data[,.I]]
    g <- ggplot(data = data) +
         geom_point(mapping = aes(x = rIndex, y = pHitMiss, colour = boolpromoter), size = 1) +
         labs(title = Title, x = Xlab, y = Ylab) +
         get_Theme()
    return(g)
}
...
#    g2 <- plot_histogram_variance_boolpromoter(data)
#    g3 <- plot_histogram_esNull_ES(esNulls, ES, ES.rank)
    #draw_plots12(g1, g3, outfile1)   
#    g4 <- plot_boxplot_varianceRank_boolpromoter(data)
