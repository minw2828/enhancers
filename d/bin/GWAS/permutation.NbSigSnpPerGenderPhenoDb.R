...
# outfile1 <- '/group/dairy/Min/geno2pheno/analyses/2016-01-19/a/out/GSEA/permutation.NbSigSnpPerGenderPhenoDb/permutation.NbSigSnpPerGenderPhenoDb.10e-08.10000.6.2016-01-19.a.png'
...
plot_histogram <- function(gender, pheno, db, NbSigSnpsPermutaion, NbSigSnpOriginal, sigThreshold, lowFill, highFill, lineColor) {
    Title <- paste(gender, pheno, db, sep = ", ")
    bin_size <- 1
...
get1plot <- function(data1, data2, loopList, index, pSigLevel, ntimes) {
    ...
    g <- plot_histogram(oneRow$gender, oneRow$pheno, oneRow$db, NbSigSnpsPermutaion, NbSigSnpOriginal, pSigLevel, oneRow$lowFill, oneRow$highFill, oneRow$lineColor)
    return(g)
}

draw_plots <- function(data1, data2, loopList, pSigLevel, ntimes, nc, outfile) {
    Title    <- paste(ntimes, 'permutation test for enrichment of GWAS significant variants within enhancer sets')
    ...
    grid.text(Title, gp = gpar(fontsize = 70, fontface = "bold"), vp = viewport(layout.pos.row = 1, layout.pos.col = 1:nc))
    grid.text(Subtitle, gp = gpar(fontsize = 60, fontface = "bold"), vp = viewport(layout.pos.row = 2, layout.pos.col = 1:nc))
    for (i in seq(3, nr)) {
        for (j in seq(1, nc)) {
            index <- nc * i + j - nc * 3
...
write_file <- function(content, outfile, boolAppend) {
    write.table(content, outfile, append = boolAppend, quote = FALSE, sep = "\t",
