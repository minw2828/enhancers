...
getCols <- function(data) {
    ...
    data[, `:=` (obsRatioQtlInDb  = obsNbQtlInDb  / totalNbSipSnp,
                 obsRatioQtlOutDb = obsNbQtlOutDb / totalNbSipSnp,
                 expRatioQtlInDb  = ratioOfProInDb,
                 expRatioQtlOutDb = 1 - ratioOfProInDb)]
    data[, oseIn := obsRatioQtlInDb - expRatioQtlInDb]
    return(data[])
}

getResult <- function(data) {
    if (nrow(data) > 1) {
        tmp <- t.test(data$obsRatioQtlInDb - data$expRatioQtlInDb)
        res <- data.table(ttestValue      = tmp$statistic,
                          ttestPvalue     = tmp$p.value,
                          ttestConf       = paste(tmp$conf.int, collapse = '; '),
                          degreeOfFreedom = tmp$parameter,
                          tpe             = 't.test')
    } else {
        res <- data.table(ttestValue      = NA,
                          ttestPvalue     = NA,
                          ttestConf       = NA,
                          degreeOfFreedom = NA,
                          tpe             = 't.test')
