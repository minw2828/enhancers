...
getCols <- function(data) {
    ...
    data[, `:=` (expNbQtlInDb  = totalNbSipSnp * ratioOfProInDb,
                 expNbQtlOutDb = totalNbSipSnp * (1 - ratioOfProInDb))]
    return(data)
}

getResult <- function(data) {
    if (nrow(data) != 0) {
        tmp <- colSums(data[, .(obsNbQtlInDb, obsNbQtlOutDb, expNbQtlInDb, expNbQtlOutDb)])
        chisqv <- (tmp['obsNbQtlInDb']  - tmp['expNbQtlInDb'])  ** 2 / tmp['expNbQtlInDb'] + 
                  (tmp['obsNbQtlOutDb'] - tmp['expNbQtlOutDb']) ** 2 / tmp['expNbQtlOutDb']
        df <- 1 
