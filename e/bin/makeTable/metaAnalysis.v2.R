...
##
## Version 2 is developed from Version 1:
## - Modify the script from Version 1, to make it work for CTCF binding motifs
##
## Author:
...
infile3   <- args[index+3]    # NCBI annotation 
infile4   <- args[index+4]    # CTCF binding motifs
...
run <- function() {
    ...
    data3 <- fread(infile3)
    data4 <- read_file(infile4)

    ah <- AnnotationHub()
    genome <- getGenome(data1, NULL, ORefseq)
    gtf <- getGtfBySpeciesRefseq(data1, ah, ORefseq, 'gene', c('gene_name', 'gene_id'))

    data34 <- getData34(data4, data3, pSigCTCF, ms)
    rgr34 <- reduce(getGranges(data34))

    res <- getResult(data2, rgr34, gtf, genome, pSigMeta, pDis1, pDis2, 2)
